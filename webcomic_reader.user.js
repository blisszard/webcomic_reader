(function() {
  ...
  try {
    GM_getValue = GM_getValue || this.GM_getValue;
  } catch (e) {
    GM_getValue = false;
  }
  try {
    GM_setValue = GM_setValue || this.GM_setValue;
  } catch (e) {
    GM_setValue = false;
  }
  try {
    GM_deleteValue = GM_deleteValue || this.GM_deleteValue;
  } catch (e) {
    GM_deleteValue = false;
  }
  try {
    GM_xmlhttpRequest = GM_xmlhttpRequest || this.GM_xmlhttpRequest;
  } catch (e) {
    GM_xmlhttpRequest = false;
  }
  try {
    GM_registerMenuCommand = GM_registerMenuCommand || this.GM_registerMenuCommand;
  } catch (e) {
    GM_registerMenuCommand = false;
  }
  try {
    GM_openInTab = GM_openInTab || this.GM_openInTab;
  } catch (e) {
    GM_openInTab = false;
  }
  ...
  */
  ...
  var paginas = [

    {
      ...
      fixurl: function(url, img, link, pos) {
      ...
      extra: [
        ['//small[@class="author-credit-name"]/../../..'],
        [/<img id="main-comic" .+?\/([^"\/]+)\.\w+"/i, 1],
        function(html, pos) {
      ...
      extra: [
        ['//title/text()'], ' - ', ['//td/font/i/b/text()'],
        ['//img[contains(@src, "/comics/archive/")]/following-sibling::table']
      ]
    }, {
      url: 'smbc-comics.com',
      img: [
      ...
      extra: [
        ['//div[@id="aftercomic"]/img[contains(@src,"/")]'],
        [
          ['.cc-newscontent:first-of-type']
        ]
      ],
      ...
      extra: [
        ['//div[@class="post"]/*', '', 0, 2],
        ['//div[@class="post"][1]//div[@class="entry"]/p[not(.//img)]', ''], '</div>'
      ]
    }, {
      url: 'erfworld.com',
      extra: [
        ...
        ['//table[@class="PxgGalleryTable"]//p[not(img)]', '']
      ]
    }, {
      ...
      extra: ['<br/>', ['//select[@id="series_select"]'],
      ...
      js: function() {
        if (keepLayout) get('comic').style.height = get('wcr_div').offsetHeight + 'px';
      },
      ...
      img: function(html, pos) {
        try {
          return selCss('#comic-1 img', html);
        } catch (e) {
      ...
      extra: [function(html, pos) {
          ...
          ['select.m']
        ], '<select id="top_chapter_list" style="display:none"></select>',
        function(html, pos) {
      ...
      next: function(html, pos) {
        ...
        try {
          ...
          var page = selCss('[name=page] [selected]', html).value + '&next';
        }
        return '?mode=view&series=' + series + '&chapter=' + chapter + '&page=' + page;
      },
      extra: ['<form name="navigate">', [
          ['.series_name']
        ],
        [
          ['.chapter_number']
      ...
      next: '.="Next"',
      extra: [
        ['//img[starts-with(@src, "/images/comics/")]', '', 1],
        ['//div[@class="comic_popup"]']
      ]
    }, {
      ...
      img: function(html, pos) {
        try {
          return selCss('div > .strip', html);
        } catch (e) {
          return selCss('.strip', html);
        }
      },
      ...
      back: function(html, pos) {
        try {
          return selCss('#previous a', html);
        } catch (e) {
          return xpath('//table[2]//td[2]//a/@href', html);
        }
      },
      next: function(html, pos) {
        try {
          return selCss('#next a', html);
        } catch (e) {
          return xpath('//table[2]//td[3]//a/@href', html);
        }
      },
      extra: [function(html, pos) {
      ...
      next: function(html, pos) {
        ...
        try {
          return xpath('//a[contains(@href, "page=' + (page + 1) + '")]/@href', html);
        } catch (e) {
          return xpath('//li[@class="right"]/a/@href', html);
        }
      }
    }, {
      url: 'comics.com',
      ...
      img: 'Strips/',
      extra: [
        ['//img[starts-with(@src, "Strips/")]', '<br/>', 1]
      ],
      ...
      extra: [
        [
          ['.comiccontainer > p[align="justify"]', '']
        ]
      ]
    }, {
      ...
      onerr: function(url, img, num) {
        ...
        n = ((n - 1) % 3) + 2; //2>3>4>2
        return {
          img: img.replace(/\/\/img\d/, '//img' + n)
        };
      }
    }, {
      url: 'e-hentai.org|exhentai.org',
      img: [
        ['#i3 a img, iframe + a img, .sni > a img']
      ],
      back: function(html, pos) {
      ...
      extra: [
        ['//div[span]'], '<span style="display:none">', ['//a[@onclick[contains(., "nl")]]'], '</span>'
      ],
      scrollx: 'R',
      onerr: function(url, img, num, pos) {
      ...
      extra: [
        [
          ['.chapter-navigation select', ' Page ']
        ]
      ],
      scrollx: 'R'
    }, {
      ...
      next: function(html, pos) {
        try {
          return xpath('//select[@class="wid60"]/option[@selected]/following-sibling::option[1]/@value', html);
        } catch (e) {
          return xpath('//p[contains(., "es el siguiente...")]/a | //strong[.="Next Chapter:"]/following-sibling::a', html);
        }
      },
      js: function(dir) {
      ...
      img: function(html, pos) {
        ...
        try {
          return selCss('a.thumbnail img', post);
        } catch (e) {}
        try {
          return selCss('img#header-img', html);
        } catch (e) {
          return '/favicon.ico';
        }
      },
      ...
      url: 'eatmanga.com',
      img: [
        ['#eatmanga_image, #eatmanga_image_big, #bigimage, .eatmanga_bigimage']
      ],
      back: function(html, pos) {
        try {
          return selCss('#page_previous:not([onclick])', html);
        } catch (e) {
      ...
      next: function(html, pos) {
        try {
          return selCss('#page_next:not([onclick])', html);
        } catch (e) {
      ...
      next: function(html, pos) {
        try {
          return selCss('#page_next.btn-view:not([onclick="next_chapter()"])', html);
        } catch (e) {
          return xpath('//select[@id="bottom_chapter_list"]/option[contains("' + link[pos] + '", @value)]/preceding-sibling::option[1]/@value',
            pos ? '<div>' + extra[0] + '</div>' : document);
        }
      ...
      extra: [
        [
          ['#asset_1 select.chapter_select']
        ], '<span>Page ', [
          ['#asset_2 select.page_select']
        ],
      ...
      extra: [
        ['//img[@id="comic_page" and not(./ancestor::div[contains(@style, "display: none;")])]', '<br/>', 1],
        [
          ['.moderation_bar']
        ],
        function(html, pos) {
          ...
          for (var i = 0; i < xs.length; i++) {
            xs[i].addEventListener('change', function() {
      ...
      back: function(html, pos) {
        try {
          return xpath('//a[.="Previous page"]', html);
        } catch (e) {
          return xpath('//span[.="Previous Chapter:"]/following-sibling::a[1]', html);
        }
      },
      next: function(html, pos) {
        try {
          return xpath('//a[.="Next page"]', html);
        } catch (e) {
          return xpath('//span[.="Next Chapter:"]/following-sibling::a[1]', html);
        }
      },
      extra: [
        [
        ...
        [
          ['.mangaread-bt .page-select select']
        ]
      ],
      scrollx: 'R'
    }, {
      ...
      back: function(html, pos) {
        try {
          ...
          var basepath = "";
          try {
            basepath = html.match(/var\s+base_?url\s*=\s*(['"])(.*?)\1\s*;/i)[2];
          } catch (e) {}
      ...
      url: '*.tiraecol.net',
      img: [
        ['img[src*="tiraecol.net/modules/comic/cache/images/"]']
      ],
      ...
      extra: [
        [
          ...
          ['div#file_dropdown_top', '']
        ],
        [
          ['div#page_dropdown_top']
        ],
        [
          ['div#reportLink']
        ],
      ...
      img: [
        ['td[background="comic_mid.gif"] img']
      ]
    }, {
      ...
      back: function(html, pos) {
        try {
          return xpath('//a[.="Previous"]', html);
        } catch (e) {
          return xpath('//a[.="Previous Chapter"]', html);
        }
      },
      ...
      back: [
        ['[alt="Previous Cartoon"]']
      ],
      next: [
        ['[alt="Next Day\'s Cartoon"]']
      ]
    }, {
      ...
      back: function(html, pos) {
        try {
          return xpath('//select[@id="pages"]/option[@selected]/preceding-sibling::option[1]/@value', html);
        } catch (e) {
      ...
      next: function(html, pos) {
        try {
          return xpath('//select[@id="pages"]/option[@selected]/following-sibling::option[1]/@value', html);
        } catch (e) {
          return xpath('//select[@id="chapters"]/option[@selected]/preceding-sibling::option[1]/@value', html);
        }
      },
      scrollx: 'R'
    }, {
      ...
      back: function(html, pos) {
        ...
        var num;
        var imgs = window.lstImages; //html.match(/lstImages\.push\(".+?"\);/g);

        try {
          num = Number(link[pos].match(/#(-?\d+)/)[1]);
        } catch (e) {
          var num = 0;
        }
      ...
      next: function(html, pos) {
        var num;
        var imgs = window.lstImages; //html.match(/lstImages\.push\(".+?"\);/g);
        this.nextImg = window.Next;
        this.lstImages = window.lstImages;
        this.currImage = window.currImage;
        this.selectChapter = $('#selectChapter').html();
        try {
          num = Number(link[pos].match(/#(-?\d+)/)[1]);
        } catch (e) {
          var num = 0;
        ...
        console.log(xpath('//select[(@id|@class)="selectChapter"]/option[@selected]/following-sibling::option[1]/@value', html) + '#1');
        // return {
        //   href: 'http://kissmanga.com/Manga/Dungeon-ni-Deai-o-Motomeru-no-wa-Machigatte-Iru-Darou-ka/Ch-076--Omen?id=383619#7'
        // };
        if (imgs.length > num + 1) return '#' + (num + 1);
        // return imgs[num];
        return xpath('//select[(@id|@class)="selectChapter"]/option[@selected]/following-sibling::option[1]/@value', html) + '#1';
      }, //[['#btnNext']],
      scrollx: 'R',
      layout: true,
      js: function(dir) {
        // $("#wcr_btn1").hide();
        // $("#wcr_btn2").prop('disabled', false);
        $("#wcr_btn2").click(function() {
          window.Previous();
        });
        window.Next = this.nextImg;
        $("#wcr_btn3").click(function() {
          window.Next();
        });
        // $("#wcr_imagen").click(function() {
        //   window.Next();
        // });
        window.lstImages = this.lstImages;
        window.currImage = this.currImage;
        $('#selectChapter').html(this.selectChapter);
        // window.SetImage = function() {
        //   $('#wcr_imagen').attr("src", window.lstImages[window.currImage]); //html("<br><img id='imgCurrent' style='cursor: pointer' src='" + window.lstImages[window.currImage] + "' />");                
        //   $('#selectPage').val(window.currImage);
        // };
      },
      fixurl: function(url, img, link, pos) {
        // if (url.indexOf('#') > 0) {
      ...
      extra: ['<span style="float:left">Chapter ', [
        ['.cbo_wpm_chp']
      ], '</span><span style="float:right">Page ', [
        ['.cbo_wpm_pag']
      ...
      back: function(html, pos) {
        try {
          return selCss('a.navi-prev-in', html);
        } catch (e) {
          return selCss('a.navi-prevchap', html);
        }
      },
      extra: [
        [
      ...
      next: '.="Next"',
      extra: [
        ['//div[contains(@class, "post-body")]//span[@title]/@title'], '<br/>', ['//u[.="News"]/following::span[./ancestor::*[contains(@class, "post-body")] and not(./ancestor::span/ancestor::*[contains(@class, "post-body")])]', '<br/>']
      ]
    }, {
      url: 'surasplace.com',
      img: function(html, pos) {
      ...
      url: 'meinmanga.com',
      img: [
        ['.pic_fragment,.pic_fragment_biggest']
      ],
      back: function(html, pos) {
        try {
          return xpath('//table[@class="pagebar"]//td[3]/select/option[@selected]/preceding-sibling::option[1]/@value', html) + '.html';
        } catch (e) {
          return xpath('//table[@class="pagebar"]//td[1]/select/option[@selected]/following-sibling::option[1]/@value', html);
        }
      },
      next: function(html, pos) {
        try {
          return xpath('//table[@class="pagebar"]//td[3]/select/option[@selected]/following-sibling::option[1]/@value', html) + '.html';
        } catch (e) {
          return xpath('//table[@class="pagebar"]//td[1]/select/option[@selected]/preceding-sibling::option[1]/@value', html);
        }
      },
      extra: [
        [
          ['.pic_fragment,.pic_fragment_biggest', '<br/>', 1]
        ], '<br/>', [
          ['.pagebar']
        ]
      ],
      ...
      back: function(html, pos) {
        try {
          return xpath('//a[img[@title="Previous Page"]]', html);
        } catch (e) {
      ...
      next: function(html, pos) {
        try {
          return xpath('//a[img[@title="Next Page"]]', html);
        } catch (e) {
      ...
      extra: [
        ['//div[contains(@class, "post-body")]/*[not(@class="separator") or contains(@style, "text-align: left")] | //div[contains(@class, "post-body")]/text()', '']
      ],
      fixurl: function(url, img, link) {
      ...
      url: '*.hentai.ms',
      img: [
        ['.index_box > table center img, .pagination img']
      ],
      scrollx: 'R'
    }, {
      ...
      back: [
        ['.comic_nav_previous_button']
      ],
      next: [
        ['.comic_nav_next_button']
      ]
    }, {
      url: '*.zizki.com',
      img: [
      ...
      back: function(html, pos) {
        try {
          return xpath('//a[./img[contains(@src, "/previous.png")]]', html);
        } catch (e) {
      ...
      next: function(html, pos) {
        try {
          return xpath('//a[./img[contains(@src, "/next.png")]]', html);
        } catch (e) {
      ...
      extra: ['<div id="wrapper"><div id="leftarea">', [
        ['#newsarea']
      ], '</div></div>'],
    }, {
      ...
      img: function(html, pos) {
        var comicNr = parseInt(link[pos].match(/\d+$/)[0]);
        if (!link.extraNotes) {
      ...
      back: 'text()="Go Back"',
      next: function(html, pos) {
        var x = selCss('font[size="5"]>a', html, true);
        return x[x.length - 1];
      },
      extra: [
        ['//table[@width="600"]']
      ...
      js: function(dir) {
        ...
        for (var i = 0; i < x.length; i++) {
          x[i].previousSibling.firstChild.click();
        }
      },
      style: '#wcr_imagen { display: none; }\np { font-size: large; }',
    }, {
      ...
      back: function(html, pos) {
        try {
          var html = html.replace(/<option[^>]*Featured.*>/i, '');
      ...
      next: function(html, pos) {
        try {
          ...
          // debugger;
          var result = xpath('//div[@class="page_select"]/select/option[@selected]/following-sibling::option[1]/@value', html);
          if (result.indexOf("feature") >= 0) {
            throw true;
          }
          return result;
        } catch (e) {
          // if (window.nextCnt === undefined) {
          //   window.nextCnt = window.current_page;
          // }
          // if (window.nextCnt >= window.total_pages) {
          var chapterUrl = xpath('//h1/a/@href', html);
      ...
      last: ['//div[@class="page_select"]/select/option[last()-1]/@value'],
      ...
      js: function(dir) {
        ...
        var divs = document.querySelectorAll('div.read_img'),
          i;
      ...
      back: function(html, pos) {
        var page = parseInt(html.match(/var rff_thisIndex = (\d+);/)[1]);
      ...
      back: function(html, pos) {
        return "/image/" + html.match(/pageid = (\d+);[^;]*[(,](\d+),\1/)[2] + "/";
      },
      next: function(html, pos) {
        return "/image/" + html.match(/pageid = (\d+);[^;]*\1,(\d+)/)[2] + "/";
      },
    }, {
  ...
  */
  ...
  var layoutDefault =
    ...
    '<button id="wcr_btn2">Back</button> ' +
    '<button id="wcr_btn3">Next</button> ' +
  ...
  function layoutIntacto() {
    ...
    if (layoutElement) img = xpath(layoutElement);
      ...
      try {
        img = xpath('//img[@src="' + src + '"]');
      } catch (e) {
        img = xpath('//img[@src="' + decodeURI(src) + '"]');
      }
    }
  ...
  var bgColor = false,
    txtColor = false; //(opcionales) fuerza el color de fondo/texto
  ...
  var achw = confBool('achw', defaultSettings.shrinkWidth),
    achh = confBool('achh', defaultSettings.shrinkHeight);
  var agrw = confBool('agrw', defaultSettings.expandWidth),
    agrh = confBool('agrh', defaultSettings.expandHeight);
    ...
    try {
      ...
      if (p) { //si esta deshabilitada esta pag p===null, si no es siempre un objeto
        ...
        if (!getImagen) {
          ...
          try {
            next = contenido(html, getNext, 0);
          } catch (e) {}
          try {
            back = contenido(html, getBack, 0);
          } catch (e) {}
  ...
  function iniciar() {
    try {
      ...
      var first, last;
      // try {
      //   first = getLink(document.documentElement.innerHTML, getFirst, 0);
      // } catch (e) {
      //   error('first: ', e);
      // }
      // try {
      //   last = getLink(document.documentElement.innerHTML, getLast, 0);
      // } catch (e) {
      //   error('last: ', e);
      // }
  ...
  function setear(html, pos, dir) {
    try {
      ...
      if (pos) {
        ...
        try {
          // debugger;
          link[poslink] = getLink(pag, dir > 0 ? getNext : getBack, pos);
        } catch (e) {
          link[poslink] = null;
        ...
        try {
          link[1] = getLink(pag, getNext, pos);
        } catch (e) {
          link[1] = null;
          error('set[' + pos + ']/link[1]: ', e);
        }
        try {
          link[-1] = getLink(pag, getBack, pos);
        } catch (e) {
          link[-1] = null;
          error('set[' + pos + ']/link[-1]: ', e);
        }
      ...
      try {
        titulo[pos] = xpath('//title', pag).innerHTML;
      } catch (ex) {
        try {
          titulo[pos] = match(html, /<title>(.+?)<\/title>/i, 1);
        } catch (e) {
          try {
            titulo[pos] = match(html, /document.title = '([^']+?)'/, 1);
          } catch (e) {
      ...
      try {
        var pagelist = "";
        var indices = Object.keys(titulo).sort(function(a, b) {
          return a - b;
        });
  ...
  function getLink(pag, getter, pos) {
    ...
    if (linkpag.indexOf('#') === 0) {
      linkpag = absUrlSharp(linkpag, pos);
    } else {
  ...
  function absUrlSharp(url, pos) {
    if (!url.indexOf('javascript:')) return null;

    // url = decodeURI(url.replace(/(^|[^#])#([^#].*|$)/, '$1').replace(/^\.\//, '').replace(/&amp;/g, '&')).trim();
    url = decodeURI(url.replace(/^\.\//, '').replace(/&amp;/g, '&')).trim();
  ...
  function cambiaPag(dir, poppedState, slidden) {
    try {
      ...
      if (keepLayout && extraElement) {
        try {
          xel = xpath(extraElement);
        } catch (e) {
          error('extraElement: ', e);
        }
      }
  ...
  function setEvt(elem, evt, fun) {
    ...
    if (!elem) return;
    if (isArray(elem))
      for (var i = 0; i < elem.length; i++) setEvt(elem[i], evt, fun);
    else if (isArray(evt))
      for (i = 0; i < evt.length; i++) setEvt(elem, evt[i], fun);
  ...
  function getData(key, defval, dominio, reloadCache) {
    ...
    var val;
    try {
      val = key ? data[dominio][key] : data[dominio];
    } catch (e) {}
  ...
  function mostrarSettings() {
    try {
      ...
      var propsSitio = {
        url: {
          desc: 'URL',
          title: "Define what sites will use these settings",
          tipos: {
            str: {
              desc: 'Beginning of URL',
              val: {
                elem: 'input',
                title: "Beginning of the url without the http://www.",
                size: 60
              }
            },
            re: {
              ...
              val: {
                elem: 'input',
                title: "Regular expression that matches the url",
                size: 60
              }
            }
          }
        ...
        img: {
          desc: 'Image',
          title: "Method for obtaining the main image",
          tipos: {
            def: {
              desc: 'Default',
              val: {
                elem: 'span',
                html: '&lt;img&gt; that is the only one with a "src" containing one of the following strings: "/comics/", "/comic/", "/strips/", "/strip/", "/archives/", "/archive/", "/wp-content/uploads/", "comics", "comic", "strips", "strip", "archives", "archive", "/manga/"'
              }
            },
            str: {
              desc: 'Beginning of src',
              val: {
                elem: 'input',
                title: "Beginning of the &quot;src&quot; attribute of the &lt;img&gt;",
                size: 60
              }
            },
            re: {
              ...
              val: {
                elem: 'input',
                title: "Regular expression that captures the whole &lt;img&gt; (or at least the &quot;src&quot; and &quot;title&quot; attributes)",
                size: 50
              },
              grp: {
                elem: 'input',
                title: "Number of the group that captured the &lt;img&gt;",
                size: 1
              }
            },
            xp: {
              ...
              val: {
                elem: 'input',
                title: "XPath query that returns the &lt;img&gt;",
                size: 60
              }
            },
            css: {
              ...
              val: {
                elem: 'input',
                title: "CSS query that returns the &lt;img&gt;",
                size: 60
              }
            },
            fn: {
              desc: 'function(html, pos)',
              val: {
                elem: 'textarea',
                title: "Function that receives the html of the page and its position relative to the starting page (0 being where you started reading), and returns the &lt;img&gt; element (either as string or object)",
                rows: 3,
                cols: 45
              }
            }
        ...
        back: {
          desc: 'Back',
          title: 'Method for obtaining the link to the previous page',
          tipos: {
            def: {
              desc: 'Default',
              val: {
                elem: 'span',
                html: '&lt;a&gt; that has the word "back" or "prev" somewhere in its innerHTML or one of its attributes'
              }
            },
            str: {
              desc: 'XPath condition',
              val: {
                elem: 'input',
                title: 'Condition for the following XPath query: //a[condition]/@href',
                size: 60
              }
            },
            re: {
              ...
              val: {
                elem: 'input',
                title: "Regular expression that captures the URL of the previous page",
                size: 50
              },
              grp: {
                elem: 'input',
                title: "Number of the group that captured the URL",
                size: 1
              }
            },
            xp: {
              ...
              val: {
                elem: 'input',
                title: "XPath query that returns the URL of the previous page, or the &lt;a&gt; element that links to it",
                size: 60
              }
            },
            css: {
              ...
              val: {
                elem: 'input',
                title: "CSS query that returns the &lt;a&gt; element that links to the previous page",
                size: 60
              }
            },
            fn: {
              ...
              val: {
                elem: 'textarea',
                title: "Function that receives the html of the page and its position relative to the starting page (0 being where you started reading), and returns the URL of the previous page",
                rows: 3,
                cols: 45
              }
            }
        ...
        next: {
          desc: 'Next',
          title: 'Method for obtaining the link to the next page',
          tipos: {
            def: {
              desc: 'Default',
              val: {
                elem: 'span',
                html: '&lt;a&gt; that has the word "next" somewhere in its innerHTML or one of its attributes'
              }
            },
            str: {
              desc: 'XPath condition',
            ...
            re: {
              ...
              val: {
                elem: 'input',
                title: "Regular expression that captures the URL of the next page",
                size: 50
              },
              grp: {
                elem: 'input',
            ...
            xp: {
              ...
              val: {
                elem: 'input',
                title: "XPath query that returns the URL of the next page, or the &lt;a&gt; element that links to it",
                size: 60
              }
            },
            css: {
              ...
              val: {
                elem: 'input',
                title: "CSS query that returns the &lt;a&gt; element that links to the next page",
                size: 60
              }
            },
            fn: {
              ...
              val: {
                elem: 'textarea',
                title: "Function that receives the html of the page and its position relative to the starting page (0 being where you started reading), and returns the URL of the next page",
                rows: 3,
                cols: 45
              }
            }
        ...
        first: {
          desc: 'First',
          title: 'Method for obtaining the link to the first page',
          tipos: {
            def: {
              desc: 'Default',
              val: {
                elem: 'span',
                html: '&lt;a&gt; that has the word "first" somewhere in its innerHTML or one of its attributes'
              }
            },
            str: {
              desc: 'XPath condition',
            ...
            re: {
              ...
              val: {
                elem: 'input',
                title: "Regular expression that captures the URL of the first page",
                size: 50
              },
              grp: {
                elem: 'input',
            ...
            xp: {
              ...
              val: {
                elem: 'input',
                title: "XPath query that returns the URL of the first page, or the &lt;a&gt; element that links to it",
                size: 60
              }
            },
            css: {
              ...
              val: {
                elem: 'input',
                title: "CSS query that returns the &lt;a&gt; element that links to the first page",
                size: 60
              }
            },
            fn: {
              ...
              val: {
                elem: 'textarea',
                title: "Function that receives the html of the page and returns the URL of the first page",
                rows: 3,
                cols: 45
              }
            }
        ...
        last: {
          desc: 'Last',
          title: 'Method for obtaining the link to the last page',
          tipos: {
            def: {
              desc: 'Default',
              val: {
                elem: 'span',
                html: '&lt;a&gt; that has the word "last", "latest", "newest" or "today" somewhere in its innerHTML or one of its attributes'
              }
            },
            str: {
              desc: 'XPath condition',
            ...
            re: {
              ...
              val: {
                elem: 'input',
                title: "Regular expression that captures the URL of the last page",
                size: 50
              },
              grp: {
                elem: 'input',
            ...
            xp: {
              ...
              val: {
                elem: 'input',
                title: "XPath query that returns the URL of the last page, or the &lt;a&gt; element that links to it",
                size: 60
              }
            },
            css: {
              ...
              val: {
                elem: 'input',
                title: "CSS query that returns the &lt;a&gt; element that links to the last page",
                size: 60
              }
            },
            fn: {
              ...
              val: {
                elem: 'textarea',
                title: "Function that receives the html of the page and returns the URL of the last page",
                rows: 3,
                cols: 45
              }
            }
        ...
        fixurl: {
          desc: 'Fix URL',
          title: 'Fix URLs coming from a link or img.src for sites that may need it (like relative URLs that don\'t behave normally, or links from http://something.com to http://www.something.com that wouldn\'t work because of cross site request limitations)',
          tipos: {
            def: {
              desc: 'Default',
              val: {
            ...
            fn: {
              desc: 'function(url, img, link, pos)',
              val: {
                elem: 'textarea',
                title: 'Function that receives an URL, and flags telling if it came from an img.src or link to another page, and returns the fixed url',
                rows: 3,
                cols: 45
              }
            }
        ...
        extra: {
          desc: 'Extra Content',
          title: 'Other content besides the main image to get from each page',
          tipos: {
            str: {
              desc: 'Literal string',
              val: {
                elem: 'input',
                title: 'HTML string, this will be output literally',
                size: 60
              }
            },
            re: {
              ...
              val: {
                elem: 'input',
                title: "Regular expression that captures the desired content",
                size: 50
              },
              grp: {
                elem: 'input',
                title: "Number of the group that captured the content",
                size: 1
              }
            },
            xp: {
              ...
              val: {
                elem: 'input',
                title: "XPath query that returns the desired content",
                size: 60
              },
              arr: {
                elem: 'select',
                html: '<option value="">First element</option><option value="1">List of elements</option>'
              },
              glue: {
                elem: 'input',
                title: 'String to put between each pair of elements returned',
                size: 20
              },
              first: {
                elem: 'input',
                title: 'Index of the first element to return (starting from 0, negative means counting from the last)',
                size: 1
              },
              last: {
                elem: 'input',
                title: 'Index of the last element to return (starting from 0, negative means counting from the last)',
                size: 1
              }
            },
            css: {
              ...
              val: {
                elem: 'input',
                title: "CSS query that returns the desired content",
                size: 60
              },
              arr: {
                elem: 'select',
            ...
            fn: {
              ...
              val: {
                elem: 'textarea',
                title: "Function that receives the html of the page and its position relative to the starting page (0 being where you started reading), and returns the desired content",
                rows: 3,
                cols: 45
              }
            }
        ...
        xelem: {
          desc: 'Extras Container',
          title: 'Element for placing the extra content when using the full layout',
          tipos: {
            def: {
              desc: 'Default',
              val: {
                elem: 'span',
                html: 'Inside the WCR container (between the image and the back/next buttons)'
              }
            },
            str: {
              desc: 'XPath',
              val: {
                elem: 'input',
                title: 'XPath query that returns the element where the extra content will be placed as its innerHTML',
                size: 60
              }
            }
          }
        ...
        layelem: {
          desc: 'Layout Container',
          title: 'Element for placing the image and the rest of the script content when using the full layout',
          tipos: {
            def: {
              desc: 'Default',
              val: {
                elem: 'span',
                html: 'Where the original image was'
              }
            },
            str: {
              desc: 'XPath',
              val: {
                elem: 'input',
                title: 'XPath query that returns the element where the content will be placed as its innerHTML',
                size: 60
              }
            }
          }
        ...
        js: {
          desc: 'Custom Action',
          title: 'Custom function to execute after each page change',
          tipos: {
            def: {
              desc: 'Default',
              val: {
            ...
            fn: {
              ...
              val: {
                elem: 'textarea',
                title: 'Function that receives the direction in which the page was changed (0 when the starting page is loaded, 1 when going forward and -1 when going backwards)',
                rows: 3,
                cols: 45
              }
            }
        ...
        scrollx: {
          desc: 'Default Horizontal Autoscroll',
          title: 'Scroll to this position of the image each time you change the page',
          tipos: {
            def: {
              desc: 'Default',
              val: {
            ...
            str: {
              desc: 'Relative to image',
              val: {
                elem: 'select',
                html: '<option value="L">Left</option><option value="R">Right</option><option value="M">Middle</option>'
              }
            },
            num: {
              desc: 'Pixels',
            ...
            fn: {
              ...
              val: {
                elem: 'textarea',
                title: "Function that returns the numbers of pixels to scroll",
                rows: 3,
                cols: 45
              }
            }
        ...
        scrolly: {
          desc: 'Default Vertical Autoscroll',
          title: 'Scroll to this position of the image each time you change the page',
          tipos: {
            def: {
              desc: 'Default',
            ...
            str: {
              ...
              val: {
                elem: 'select',
                html: '<option value="U">Top</option><option value="D">Bottom</option><option value="M">Middle</option>'
              }
            },
            num: {
              desc: 'Pixels',
        ...
        layout: {
          desc: 'Default Layout',
          title: 'Layout to use when no custom layout settings are defined for this site',
          tipos: {
            def: {
              desc: 'Default',
              val: {
            ...
            bool: {
              ...
              val: {
                elem: 'select',
                html: '<option value="false">Minimalistic</option><option value="true">Keep original</option>'
              }
            }
          }
        }
      ...
      var opsGeneral = {
        clickImgNavigates: {
          desc: 'Click image to navigate',
          title: 'If enabled, clicking the image will let you go to the next or previous page',
        ...
        click_img_izq: {
          desc: 'Click left half of<br/>image to go back',
          title: 'If enabled, clicking the left half of the image will take you to the previous page, and the right half to the next one. Otherwise, clicking anywhere will always take you to the next page',
        ...
        flipControlsManga: {
          desc: 'Flip controls<br/>for mangas',
          title: 'If enabled, flips the controls (L/R arrows, L/R image click, back/next buttons) for mangas or other right-to-left content',
        ...
        overwrite_links: {
          desc: 'Overwrite links',
          title: 'If enabled, overwrites the original back/next links (when using the original layout) to work like the script\'s buttons',
          def: '1',
          vals: {
            '0': 'Disabled',
            '1': 'Enabled'
        ...
        goToBookmark: {
          desc: 'Go to bookmark',
          title: 'If you have 1 bookmark saved for a site, asks you if you want to go there when you visit the site',
        ...
        scroll_rate: {
          desc: 'Scroll rate',
          title: 'Number of pixels to scroll when using the keyboard',
          def: '50'
        },
        b64_images: {
          desc: 'Force cache (experimental)',
          title: 'Chache images as base64 strings, so the browser doesn\'t unload them to save memory',
          def: '0',
          vals: {
            '0': 'Disabled',
            '1': 'Enabled'
        ...
        useHistoryAPI: {
          desc: 'Use browser history',
          title: 'Changes the URL and keeps track of the visited pages in the browser history, so you can navigate with the browser\'s back/forward buttons as usual',
          def: '1',
          vals: {
            '0': 'Disabled',
            '1': 'Enabled'
        ...
        moveWhileLoading: {
          desc: 'Force loading next page',
          title: 'Lets you move to the next or previous page before the image for that page has finished loading',
        ...
        _grp_fit: {
          desc: 'AutoZoom',
          title: 'Automatically zoom the image, either shrinking or expanding it to make it fit in the screen'
        },
        fit: {
          desc: 'Fit image to screen',
          title: 'Apply options below to fit the image to the screen (if none of them are selected and you enable this option, you will be prompted to select the settings the first time you visit each site). This setting can also be toggled for this site with a keyboard shortcut (+ by default)',
        ...
        achw: {
          desc: 'Shrink to fit width',
          title: 'If the image is wider than the window, it will be shrunk to fit the screen without needing to scroll',
        ...
        achh: {
          desc: 'Shrink to fit height',
          title: 'If the image is longer than the window, it will be shrunk to fit the screen without needing to scroll',
        ...
        agrw: {
          desc: 'Expand to fit width',
          title: 'If the image is smaller than the window, it will be expanded to fit the screen',
        ...
        maxScale: {
          desc: 'Max scale',
          title: 'Maximum scale (as a percentage, > 100) to which the image should be expanded (leave blank for no limit)'
        },
        minScale: {
          desc: 'Min scale',
          title: 'Minimum scale (as a percentage, < 100) to which the image should be shrunk (leave blank for no limit)'
        },
        maxScaleReset: {
          desc: 'Over max scale',
          title: 'Action to be taken when the AutoZoom would expand the image over the max scale',
          def: '0',
          vals: {
        ...
        minScaleReset: {
          desc: 'Under min scale',
          title: 'Action to be taken when the AutoZoom would shrink the image over the min scale',
          def: '0',
          vals: {
        ...
        _grp_prefetch: {
          desc: 'Page Preloading',
          title: 'Adjust the number of pages to preload in each direction'
        },
        prefetch_der: {
          desc: 'Forward',
          title: 'The number of next pages to preload (>0)',
          def: defaultSettings.prefetchNext
        },
        prefetch_izq: {
          desc: 'Backwards',
          title: 'The number of previous pages to preload (>0)',
          def: defaultSettings.prefetchBack
        },
        prefetch_start_der: {
          desc: 'Initial forward',
          title: 'The number of next pages to preload (>0) when the page is first loaded (to avoid wasting bandwith if you only wanted to see that page)',
          def: defaultSettings.prefetchNextStart
        },
        prefetch_start_izq: {
          desc: 'Initial backwards',
          title: 'The number of previous pages to preload (>0) when the page is first loaded (to avoid wasting bandwith if you only wanted to see that page)',
          def: defaultSettings.prefetchBackStart
        },
        prefetchNoNext: {
          desc: 'Prefetch when no next page',
          title: 'Disable this to stop preloading the previous page when visiting the last page (ie, the next page was not found)',
      ...
      var opsLayout = {
        ...
        botones: {
          desc: 'Buttons',
          title: 'Show or hide all the script\'s buttons (back/next, bookmarks, settings, etc...). This setting can also be toggled for this site with a keyboard shortcut (Shift + - by default)',
        ...
        dim: {
          desc: 'Screen Dimmer',
          title: 'Add a shadow to the rest of the site so the image (or script content) gets a better focus',
          def: '0',
          vals: {
            '0': 'Disabled',
        ...
        _grp_border: {
          desc: 'Border',
          title: 'Space to leave around the image (affects AutoScroll and AutoZoom)'
        },
        bordex: {
          desc: 'Horizontal border',
          title: 'Extra pixels to the left/right of the image',
          def: defaultSettings.borderLR
        },
        bordey: {
          desc: 'Vertical border',
          title: 'Extra pixels to the top/bottom of the image',
          def: defaultSettings.borderUD
        },

        _grp_cursor: {
          desc: 'Cursors',
          title: 'Change the cursor according to the current state'
        },
        chcursor_img: {
          desc: 'Change over image',
          title: 'Enable/Disable this to see a different cursor over the image depending on the state, or always the same one',
          def: '1',
          vals: {
            '0': 'Disabled',
            '1': 'Enabled'
        ...
        chcursor_btns: {
          desc: 'Change over buttons',
          title: 'Enable/Disable this to see a different cursor over the back/next buttons depending on the state, or always the same one',
          def: '1',
          vals: {
            '0': 'Disabled',
            '1': 'Enabled'
        ...
        cursor_back: {
          desc: 'Previous page',
          title: 'Cursor for the previous page',
          def: '1',
          vals: cursores
        },
        cursor_next: {
          desc: 'Next page',
          title: 'Cursor for the next page',
          def: '2',
          vals: cursores
        },
        cursor_loading: {
          desc: 'Loading',
          title: 'Cursor for when the next page is loading',
          def: 'progress',
          vals: cursores
        },
        cursor_nolink: {
          desc: 'No link',
          title: 'Cursor for when there is no next page',
          def: 'not-allowed',
          vals: cursores
        },
        cursor_noimg: {
          desc: 'No image',
          title: 'Cursor for when there is a next page but it has no image',
          def: 'pointer',
          vals: cursores
        },
        cursor_custom_3: {
          desc: 'Custom cursor #1',
          title: 'Custom image to use in the above options, either as an url or base64 (suggested size of 32x32 px, hotspot is at 16,16)'
        },
        cursor_custom_4: {
          desc: 'Custom cursor #2',
          title: 'Custom image to use in the above options, either as an url or base64 (suggested size of 32x32 px, hotspot is at 16,16)'
        }
      };
  ...
  function cambiaTipo(sel) {
    var clave = sel.id.substr('wcr_sitio_tipo_'.length),
      tipo = sel.value;
  ...
  function tdsConfSitio(p, prop) {
    ...
    for (t in prop.tipos)
      for (var v in prop.tipos[t]) {
        if (v == 'desc') continue;
        tds[1] += '<' + prop.tipos[t][v].elem + ' id="wcr_sitio_valor_' + p + '_' + t + '_' + v + '"';
        for (var a in prop.tipos[t][v])
          if (a != 'elem' && a != 'html') tds[1] += ' ' + a + '="' + prop.tipos[t][v][a] + '"';
